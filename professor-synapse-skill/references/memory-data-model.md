...
- **`dream`** (read-only) surfaces the raw material: **clusters** — connected components of the co-recall graph over edges whose *decayed* weight clears `DREAM_EDGE_FLOOR`, among live records (non-`dropped`, non-consolidated), returned densest-first for components of at least `DREAM_MIN_CLUSTER` members — and **persona material** — the current profile plus the newest `DREAM_PERSONA_FACTS` high-confidence `fact`s. It writes nothing.
- **`consolidate --from <ids> --text "<summary>"`** folds a cluster into a new `insight`: it writes the summary record, `link`s it to each source atom (a `manual` edge, so drill-down works), and stamps each atom's `consolidated_into`. The atoms are then held out of primary recall (see search, above) but kept as evidence, reachable one hop behind the summary. Guards: every `--from` id must exist, not be `dropped`, and not already be consolidated.
